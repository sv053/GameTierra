...
public class PaymentProcessingMock implements PaymentProcessingApi {
    ...
    public PaymentResponse processPayment(PaymentRequest paymentRequest) {
        ...
        if (paymentRequest.getAmount().compareTo(CARD_LIMIT) > 0) {
            return new PaymentResponse(TRANSACTION_ID, false, ResponseError.INSUFFICIENT_FUNDS);
        }
        if (card.getCardholderName().startsWith("err")) {
            return new PaymentResponse(TRANSACTION_ID, false, ResponseError.FORMAT_ERROR);
        }
        if (card.getExpireDate().isBefore(LocalDate.now())) {
            return new PaymentResponse(TRANSACTION_ID, false, ResponseError.EXPIRED_CARD);
        }
        if (16 != card.getCardNumber().toString().length()) {
            return new PaymentResponse(TRANSACTION_ID, false, ResponseError.INVALID_CARD_NUMBER);
        }
        Integer cvc = card.getCvcCode();
        if (cvc.toString().length() < 3) {
            return new PaymentResponse(TRANSACTION_ID, false, ResponseError.CVC_ERROR);
        }
        return new PaymentResponse(TRANSACTION_ID, true, null);
    }
}
